...
import { expenseCategoryLabel, formatDateTime, formatMoney } from '@/utils/format'
...
const total = computed(() =>
  filtered.value.reduce((sum, e) => sum + Number(e.amount || 0), 0),
)
  ...
  try {
    ...
    error.value =
      err.response?.status === 403
        ? 'Only the shop owner can manage expenses.'
...
<template>
  <div>
    <PageHeader title="Expenses" subtitle="Owner-only ledger for shop costs.">
      <template #actions>
    ...
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-[220px] flex-1">
        <AppInput v-model="search" placeholder="Search expenses…" />
      </div>
      <div class="border border-line bg-surface px-4 py-2 text-sm">
        <span class="text-stone">Showing total · </span>
        <span class="font-semibold text-ink">{{ formatMoney(total) }}</span>
      </div>
    </div>
    ...
    <EmptyState
      ...
      description="Track rent, utilities, salary, and restock costs here."
    >
      <AppButton @click="openCreate">Add expense</AppButton>
    </EmptyState>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full text-left text-sm">
          ...
          <tbody>
            ...
            >
              <td class="px-4 py-3 whitespace-nowrap text-stone">
                {{ formatDateTime(expense.created_at) }}
              </td>
              <td class="px-4 py-3 font-medium text-ink">{{ expense.description }}</td>
              <td class="px-4 py-3">
                <AppBadge tone="teal">{{ expenseCategoryLabel(expense.category) }}</AppBadge>
              </td>
              <td class="px-4 py-3 font-semibold tabular-nums">{{ formatMoney(expense.amount) }}</td>
