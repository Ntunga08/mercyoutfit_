...
const filtered = computed(() => {
  ...
  return products.value.filter((p) =>
    `${p.name} ${p.category_name || ''} ${p.description || ''}`.toLowerCase().includes(q),
  )
})
...
async function saveProduct() {
  ...
  try {
    ...
    if (productForm.id) await productsApi.update(productForm.id, payload)
    else await productsApi.create(payload)

    productModal.value = false
    await load()
  } catch (err) {
    formError.value = apiErrorMessage(err, 'Could not save product.')
  } finally {
    saving.value = false
  }
}
...
async function saveVariant() {
  ...
  try {
    ...
    formError.value = apiErrorMessage(err, 'Could not add variant.')
  } finally {
    saving.value = false
  }
}
...
async function saveCategory() {
  ...
  try {
    ...
    formError.value = apiErrorMessage(err, 'Could not create category.')
  } finally {
    saving.value = false
  }
}
...
async function toggleActive(product) {
  try {
    ...
    error.value = apiErrorMessage(err, 'Could not update product.')
  }
}
...
<template>
  <div>
    <PageHeader title="Product catalog" subtitle="Owner-only: categories, products, variants, images.">
      <template #actions>
        <AppButton
          variant="ghost"
          @click="categoryModal = true; formError = ''; categoryForm.name = ''"
        >
          New category
        </AppButton>
    ...
    <div class="mb-5"><AppInput v-model="search" placeholder="Search products…" /></div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      >
        <div class="relative aspect-[4/3] bg-canvas">
          ...
          <div v-else class="flex h-full items-center justify-center bg-navy font-display text-3xl text-white/80">
        ...
        <div class="p-4">
          <p class="text-xs font-semibold text-teal">{{ product.category_name || 'Uncategorized' }}</p>
          <h3 class="mt-1 font-display text-xl">{{ product.name }}</h3>
          <p v-if="product.description" class="mt-1 line-clamp-2 text-sm text-stone">{{ product.description }}</p>
          <div class="mt-4 space-y-2">
            <div
              v-for="v in product.variants"
              :key="v.id"
              class="flex items-center justify-between rounded-xl bg-mist px-3 py-2 text-sm"
            >
              <span>
    ...
    <AppModal :open="productModal" :title="productForm.id ? 'Edit product' : 'New product'" @close="productModal = false">
      <form class="space-y-4" @submit.prevent="saveProduct">
        ...
        <label class="block">
          <span class="mb-1.5 block text-sm font-medium">Description</span>
          <textarea
